...
public partial class StartPage : global::Avalonia.Controls.UserControl
{
    ...
    {
        // Resolve service dependencies from the app's root DI container
    ...
    // ── Card factory ───────────────────────────────────────────────
    private Border CreateRecentCard(string filePath, MainViewModel vm, int index)
    {
        var fileName = System.IO.Path.GetFileName(filePath);
        var ext      = System.IO.Path.GetExtension(filePath).ToLowerInvariant();
        var isVideo = _mediaFileService.IsVideoFile(filePath);
    ...
    {
        var accentDim = new SolidBrush(AvColor.FromArgb(31, 201, 169, 110));
        var bg = active ? accentDim : null;
        if (DropZone is not null)     DropZone.Background = bg;
        if (DropZoneNarrow is not null) DropZoneNarrow.Background = bg;
    }
    ...
    private void OnGlobalDragEnter(object? sender, DragEventArgs e)
    {
        ...
        if (DropTarget is not null) DropTarget.Opacity = 1;
    }
    ...
    private void OnGlobalDragLeave(object? sender, DragEventArgs e)
    {
        ...
        if (_dragCounter == 0)
        {
            ...
            if (DropTarget is not null) DropTarget.Opacity = 0;
        }
    }
