...
public partial class MainViewModel
{
    ...
    public async Task OpenFolderFromPathAsync(string folderPath)
    {
        ...
        if (files.Length == 0)
        {
            OnDropResult?.Invoke(this, "No media files found in folder.");
            return;
        }
        await OpenFiles(files);
    }
    ...
    public async Task OpenDroppedFilesAsync(string[]? paths)
    {
        if (paths == null || paths.Length == 0)
        {
            OnDropResult?.Invoke(this, "No files to open.");
            return;
        }
        ...
        if (sorted.Length == 0)
        {
            OnDropResult?.Invoke(this, "No media files found in the selection.");
            return;
        }
        ...
        if (IsPlaying || IsPaused)
        {
            ...
            OnDropResult?.Invoke(this, $"Added {sorted.Length} file(s) to playlist.");
        }
        else
        {
            // Idle — replace playlist and start fresh
            await OpenFiles(sorted);
            OnDropResult?.Invoke(this, $"Opening {sorted.Length} file(s).");
        }
    }
