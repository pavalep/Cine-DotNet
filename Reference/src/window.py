...
# Try to load EGL library (Linux .so or Windows .dll)
for lib_name in ["libEGL.so.1", "libEGL-1.dll", "libEGL.dll"]:
    try:
        ...
        print(f"Loaded EGL from {lib_name}")
        break
    except OSError:
        continue

# If EGL is not available (Windows), try WGL
if not egl_get_proc_address:
    try:
        # On Windows, use wglGetProcAddress from opengl32.dll
        ...
        egl_get_proc_address = wglGetProcAddress
        print("Using WGL (wglGetProcAddress) for OpenGL")
    except Exception as e:
        print(f"Could not load WGL: {e}")

GL_FRAMEBUFFER_BINDING = 0x8CA6
# Try to load OpenGL library (Linux .so or Windows .dll)
for lib_name in ["libGL.so.1", "libopengl32.dll", "opengl32.dll", "libGL-1.dll"]:
    try:
        ...
        print(f"Loaded OpenGL from {lib_name}")
        break
    except OSError:
        continue

# Try to load GTK library (Linux .so or Windows .dll)
for lib_name in ["libgtk-4.so.1", "libgtk-4-1.dll"]:
    try:
        gtk = ctypes.CDLL(lib_name)
        print(f"Loaded GTK from {lib_name}")
        break
    except OSError:
        continue
...
class CineWindow(Adw.ApplicationWindow):
    ...
    def __init__(self, **kwargs):
        ...
        self.use_software_render: bool = False
    ...
    def _get_display_param(self):
        ...
        try:
            if GdkWayland and gtk and isinstance(display, GdkWayland.WaylandDisplay):
            ...
            elif GdkX11 and gtk and isinstance(display, GdkX11.X11Display):
    ...
    def _on_realize_area(self, area):
        ...
        if not egl_get_proc_address:
            print("ERROR: No OpenGL proc address function available")
            print("Cannot initialize video rendering")
            return

        print("Initializing OpenGL rendering")
        ...
        if not display_param:
            print("Note: No display params (normal on Windows)")
        ...
        try:
            ...
            self.use_software_render = False
            print("OpenGL rendering initialized successfully")
        except Exception as e:
            print(f"OpenGL initialization failed: {e}")
            print("Video rendering will not work")
            return
    ...
    def _on_render_area(self, area, _context):
        ...
        try:
            ...
            if not hasattr(self, '_render_error_count'):
